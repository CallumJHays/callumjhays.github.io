...
export function GithubActivity() {
  ...
  useEffect(() => {
    // @ts-ignore
    OrigGitHubActivity.feed({
      username: "callumjhays",
  ...
  var methods = {
    ...
    getMessageFor: function (data) {
      ...
      if (p.ref) {
        ...
        data.branchLink =
          methods.renderGitHubLink(
            data.repo.name + "/tree/" + data.branch,
            data.branch
          ) + " at ";
      }
      ...
      if (p.commits) {
        ...
        if (length === 2) {
          ...
          data.commitsMessage = Mustache.render(
            '<a href="https://github.com/{{repo}}/compare/{{shaDiff}}">{{length}} more ' +
              ...
              " &raquo;</a>",
            { repo: data.repo.name, shaDiff: shaDiff, length: p.size - 2 }
          );
        }
        ...
        p.commits.forEach(function (d, i) {
          ...
          if (i < 2) {
            d.shaLink = methods.renderGitHubLink(
              data.repo.name + "/commit/" + d.sha,
              d.sha.substring(0, 6),
              "gha-sha"
            );
            d.committerGravatar = Mustache.render(
      ...
      if (p.pull_request) {
        ...
        data.pullRequestLink = methods.renderLink(
          pr.html_url,
          data.repo.name + "#" + pr.number
        );
        data.mergeMessage = "";
        ...
        if (p.pull_request.merged) {
          ...
          var message =
            ...
            " with {{a}} " +
            pluralize("addition", pr.additions) +
            " and {{d}} " +
            pluralize("deletion", pr.deletions);
          data.mergeMessage = Mustache.render(
            '<br><small class="gha-message-merge">' + message + "</small>",
            { c: pr.commits, a: pr.additions, d: pr.deletions }
          );
        }
      }
      ...
      if (p.comment && p.comment.pull_request_url) {
        var title =
          data.repo.name + "#" + p.comment.pull_request_url.split("/").pop();
      ...
      if (p.comment && p.comment.body) {
        ...
        if (p.comment.html_url && p.comment.commit_id) {
          var title =
            data.repo.name + "@" + p.comment.commit_id.substring(0, 10);
          data.commentLink = methods.renderLink(p.comment.html_url, title);
        }
      }

      if (data.type === "ReleaseEvent") {
        data.tagLink = methods.renderLink(
          p.release.html_url,
          p.release.tag_name
        );
        data.zipLink = methods.renderLink(
          p.release.zipball_url,
      ...
      if (data.type === "GollumEvent") {
        ...
        data.message =
          ...
          data.actionType.slice(1) +
          " ";
        data.message += methods.renderGitHubLink(page.html_url, page.title);
      }

      if (data.type === "FollowEvent")
        data.targetLink = methods.renderGitHubLink(p.target.login);
      if (data.type === "ForkEvent")
        data.forkLink = methods.renderGitHubLink(p.forkee.full_name);
      if (data.type === "MemberEvent")
        data.memberLink = methods.renderGitHubLink(p.member.login);

      if (p.gist) {
        data.actionType =
          p.action === "fork" ? p.action + "ed" : p.action + "d";
        data.gistLink = methods.renderLink(
          p.gist.html_url,
          "gist: " + p.gist.id
        );
      }
      ...
      if (
        ...
        ["repository", "branch", "tag"].indexOf(p.ref_type) >= 0
      ) {
    ...
    getHeaderHTML: function (data) {
      ...
      data.gravatarLink = methods.renderLink(
        data.html_url,
      ...
      request.onreadystatechange = function () {
        if (request.readyState === 4) {
          if (request.status >= 200 && request.status < 300) {
            ...
            callback(
              "request for " + url + " yielded status " + request.status
            );
          }
        }
      };
  ...
  obj.feed = function (options) {
    ...
    methods.getOutputFromRequest(eventsUrl, function (error, output) {
      if (error) {
        ...
        var limit =
          options.limit != "undefined" ? parseInt(options.limit, 10) : null;
...
var templates = {
    ...
    SingleLineActivity:
      '<div class="gha-activity gha-small">\
                           <div class="gha-activity-icon"><span class="octicon octicon-{{icon}}"></span></div>\
                           <div class="gha-message"><div class="gha-time">{{{timeString}}}</div>{{{userLink}}} {{{message}}}</div>\
                           <div class="gha-clear"></div>\
                         </div>',
